  ...
  if (authLoading || loading) {
    return (
      <div className="min-h-screen bg-background ">
        <SiteHeader />
        <div className="container py-20 flex justify-center"><div className="w-8 h-8 border-2 border-primary border-t-transparent rounded-xl animate-spin" /></div>
      </div>
    );
  }
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}>
            ...
            <div className="space-y-4">
              <input type="text" placeholder="Article title"
                ...
                className="w-full px-4 py-3 rounded-xl bg-card border border-border text-foreground font-display text-lg font-bold focus:outline-none focus:ring-2 focus:ring-primary/50 transition-all" />

              <textarea placeholder="Short excerpt (1-2 sentences)"
                value={editingArticle?.excerpt || ''}
                onChange={e => setEditingArticle(prev => ({ ...prev, excerpt: e.target.value }))}
                rows={2}
                className="w-full px-4 py-2.5 rounded-xl bg-card border border-border text-foreground font-body text-sm focus:outline-none focus:ring-2 focus:ring-primary/50 transition-all resize-none" />
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  ...
                  <select value={editingArticle?.category_id || ''}
                    ...
                    className="w-full px-3 py-2.5 rounded-xl bg-card border border-border text-foreground font-body text-sm focus:outline-none focus:ring-2 focus:ring-primary/50 transition-all">
              ...
              <div className="flex gap-3 pt-4">
                <button onClick={() => handleSave('draft')}
                  className="px-5 py-2.5 bg-card border border-border rounded-xl font-body text-sm font-medium text-foreground hover:bg-muted/50 transition-all duration-300">
                  Save Draft
                </button>
                <button onClick={() => handleSave('approved')}
                  className="px-5 py-2.5 bg-primary text-primary-foreground rounded-xl font-body text-sm font-medium hover:bg-foreground hover:text-background transition-all duration-300 flex items-center gap-1.5">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}
          ...
          <div className="flex gap-2">
            <button onClick={() => { setEditingArticle({}); setView('editor'); }}
              className="px-4 py-2.5 bg-primary text-primary-foreground rounded-xl font-body text-sm font-medium hover:bg-foreground hover:text-background transition-all duration-300 flex items-center gap-1.5">
              <Plus className="w-4 h-4" /> New Article
            </button>
            <button onClick={signOut}
              className="px-4 py-2.5 bg-card border border-border rounded-xl font-body text-sm font-medium text-foreground hover:bg-muted/50 transition-all duration-300 flex items-center gap-1.5">
          ...
          {(['draft', 'pending', 'approved', 'rejected'] as ArticleStatus[]).map((status, i) => {
            ...
            return (
              <motion.div key={status} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.1 }}
                className={`bg-card border border-border rounded-xl p-4 bg-card ${cfg.gradient} stat-card transition-all duration-500`}>
          ...
          {articles.map((article, i) => {
            ...
            return (
              <motion.div key={article.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }}
                className="bg-card border border-border rounded-xl p-4 flex items-start gap-4 hover:border-accent transition-all duration-300">
                {article.cover_image && (
                  <img src={article.cover_image} alt="" className="w-20 h-16 rounded-xl object-cover flex-shrink-0 hidden sm:block" />
                )}
                <div className="flex-1 min-w-0">
                  <h3 className="text-base font-display font-semibold text-foreground line-clamp-2">{article.title}</h3>
                  <div className="flex items-center gap-2 mt-1">
                    <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-xl text-xs font-body font-medium ${cfg.text} bg-current/10`}
                ...
                <div className="flex gap-1 flex-shrink-0">
                  {article.status === 'approved' && (
                    <Link to={`/article/${article.slug}`} className="p-2 rounded-xl hover:bg-muted/50 transition-all duration-300 text-muted-foreground hover:text-foreground">
                      <Eye className="w-4 h-4" />
                    </Link>
                  )}
                  <button onClick={() => { setEditingArticle(article); setView('editor'); }}
                    className="p-2 rounded-xl hover:bg-muted/50 transition-all duration-300 text-muted-foreground hover:text-foreground">
                    <Edit className="w-4 h-4" />
                  </button>
                  <button onClick={() => handleDelete(article.id)}
                    className="p-2 rounded-xl hover:bg-destructive/20 transition-all duration-300 text-destructive">

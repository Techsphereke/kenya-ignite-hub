...
import { FileText, Clock, CheckCircle, XCircle, Plus, Eye, Send, Search, ImagePlus, ChevronDown, MessageSquare, UserRound, Images, ArrowRight, CircleCheck, CircleAlert, CircleX } from 'lucide-react';
import { motion } from 'framer-motion';
import type { Database } from '@/integrations/supabase/types';
import { analyzeArticleSeo, getSeoDescription, getSeoTitle, seoStatusClass, seoStatusLabel } from '@/lib/article-seo';
...
const Dashboard = () => {
  ...
  const handleSave = async (status: ArticleStatus) => {
    ...
    const { title, excerpt, content, cover_image, category_id, tags, published_at, seo_title, meta_description, focus_keyphrase, canonical_url } = editingArticle;
    if (!title?.trim()) { toast.error('Title is required'); return; }
    if (canonical_url?.trim()) {
      try { if (new URL(canonical_url.trim()).protocol !== 'https:') throw new Error(); }
      catch { toast.error('Canonical URL must be a complete HTTPS address'); return; }
    }
    const live = status === 'approved' || status === 'pending';
    const payload = { title: title.trim(), slug: editingArticle.slug || '', excerpt: excerpt || '', content: content || '', cover_image: cover_image || null, category_id: category_id || null, tags: tags || [], author_id: user.id, status, published_at: published_at ? new Date(published_at).toISOString() : (live ? new Date().toISOString() : null), seo_title: seo_title?.trim() || null, meta_description: meta_description?.trim() || null, focus_keyphrase: focus_keyphrase?.trim() || null, canonical_url: canonical_url?.trim() || null };
  ...
  const seoAnalysis = analyzeArticleSeo(editingArticle || {});
  const previewTitle = getSeoTitle(editingArticle || {}) || 'Your search title will appear here';
  const previewDescription = getSeoDescription(editingArticle || {}) || 'Add a meta description or excerpt to preview how this story may appear in search.';
      ...
      <main className="p-4 md:p-7">
        <div className="mx-auto grid max-w-[1220px] gap-5 lg:grid-cols-[minmax(0,1fr)_300px]">
          ...
          <aside className="space-y-4">
            ...
            <div className="newsroom-panel">
              <div className="flex items-center justify-between border-b border-newsroom-line px-4 py-3"><h2 className="font-newsroom-heading text-sm font-semibold">SEO</h2><span className={`flex items-center gap-1 text-xs font-semibold ${seoStatusClass[seoAnalysis.status]}`}>{seoAnalysis.status === 'good' ? <CircleCheck className="h-4 w-4" /> : seoAnalysis.status === 'needs-improvement' ? <CircleAlert className="h-4 w-4" /> : <CircleX className="h-4 w-4" />}{seoStatusLabel[seoAnalysis.status]}</span></div>
              <div className="space-y-4 p-4">
                <div className="border border-newsroom-line bg-newsroom-surface p-3"><p className="truncate text-xs text-newsroom-success">jubachronicles.com › article</p><p className="mt-1 line-clamp-2 text-base text-newsroom-blue">{previewTitle}</p><p className="mt-1 line-clamp-2 text-xs leading-5 text-newsroom-muted">{previewDescription}</p></div>
                <label className="block text-xs font-semibold">Focus keyphrase<input value={editingArticle?.focus_keyphrase || ''} onChange={e => setEditingArticle(prev => ({ ...prev, focus_keyphrase: e.target.value }))} className="newsroom-field mt-1" placeholder="Main phrase readers search" /></label>
                <label className="block text-xs font-semibold">SEO title <span className="float-right font-normal text-newsroom-muted">{(editingArticle?.seo_title || '').length}/60</span><input value={editingArticle?.seo_title || ''} maxLength={70} onChange={e => setEditingArticle(prev => ({ ...prev, seo_title: e.target.value }))} className="newsroom-field mt-1" placeholder={editingArticle?.title || 'Uses the post title by default'} /></label>
                <label className="block text-xs font-semibold">Meta description <span className="float-right font-normal text-newsroom-muted">{(editingArticle?.meta_description || '').length}/160</span><textarea value={editingArticle?.meta_description || ''} maxLength={180} onChange={e => setEditingArticle(prev => ({ ...prev, meta_description: e.target.value }))} rows={4} className="newsroom-field mt-1 resize-none" placeholder="Uses the excerpt by default" /></label>
                <label className="block text-xs font-semibold">Canonical URL <span className="font-normal text-newsroom-muted">(optional)</span><input value={editingArticle?.canonical_url || ''} onChange={e => setEditingArticle(prev => ({ ...prev, canonical_url: e.target.value }))} type="url" className="newsroom-field mt-1" placeholder="https://jubachronicles.com/article/..." /></label>
                <div className="border-t border-newsroom-line pt-3"><div className="mb-2 flex items-center justify-between text-xs"><strong>{seoAnalysis.score}/100</strong><span className={seoStatusClass[seoAnalysis.status]}>{seoAnalysis.passed} of {seoAnalysis.checks.length} checks passed</span></div><div className="space-y-1.5">{seoAnalysis.checks.map(check => <div key={check.label} className="flex items-start gap-2 text-xs text-newsroom-muted">{check.passed ? <CircleCheck className="mt-0.5 h-3.5 w-3.5 shrink-0 text-newsroom-success" /> : <CircleAlert className="mt-0.5 h-3.5 w-3.5 shrink-0 text-newsroom-warning" />}<span>{check.label}</span></div>)}</div></div>
              </div>
            </div>
          </aside>
        </div>

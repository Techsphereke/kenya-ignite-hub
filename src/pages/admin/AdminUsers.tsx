        ...
        <div className="space-y-3">
          {users.map((user, i) => (
            <motion.div key={user.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }}
              className="bg-card border border-foreground/20 rounded-sm p-4 flex items-center gap-4 hover:border-foreground transition-all duration-300">
              {user.avatar_url ? (
                <img src={user.avatar_url} alt="" className="w-10 h-10 rounded-sm object-cover flex-shrink-0 ring-2 ring-border/30" />
              ) : (
                <div className="w-10 h-10 rounded-sm bg-card from-primary/20 to-accent/20 flex items-center justify-center flex-shrink-0">
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="flex flex-wrap gap-1.5 mt-1">
                  {user.roles.map(role => (
                    <span key={role} className={`px-2 py-0.5 rounded-sm text-xs font-body font-medium ${roleColors[role]}`}>
                      {role}
                    </span>
                  ))}
                </div>
                ...
                {(['admin', 'editor', 'author'] as AppRole[]).map(role => {
                  ...
                  return (
                    <button key={role} onClick={() => toggleRole(user.user_id, role, has)}
                      ...
                      className={`px-2.5 py-1 rounded-sm text-xs font-body font-medium transition-all duration-300 ${

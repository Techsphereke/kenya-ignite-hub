  ...
  if (variant === 'compact') {
    return (
      <Link to={`/article/${article.slug}`} className="flex gap-3 group py-3 border-b border-border last:border-0">
        {article.cover_image && <img src={article.cover_image} alt={article.title} className="w-20 h-20 rounded-md object-cover flex-shrink-0" loading="lazy" />}
  ...
  if (variant === 'horizontal') {
    return (
      <Link to={`/article/${article.slug}`} className="flex gap-4 group">
        {article.cover_image && <img src={article.cover_image} alt={article.title} className="w-32 h-24 md:w-48 md:h-32 rounded-md object-cover flex-shrink-0" loading="lazy" />}
        <div className="flex-1 min-w-0">
          {article.category_name && <span className="text-xs font-body font-semibold text-primary uppercase">{article.category_name}</span>}
  ...
  if (variant === 'featured') {
    return (
      <Link to={`/article/${article.slug}`} className="group relative block rounded-lg overflow-hidden">
        <img src={article.cover_image || '/placeholder.svg'} alt={article.title} className="w-full h-64 md:h-80 object-cover" loading="lazy" />
  ...
  return (
    <Link to={`/article/${article.slug}`} className="group block">
      <div className="rounded-lg overflow-hidden bg-card border border-border hover:shadow-lg transition-shadow">
        <img src={article.cover_image || '/placeholder.svg'} alt={article.title} className="w-full h-44 object-cover" loading="lazy" />
        <div className="p-4">
          {article.category_name && <span className="text-xs font-body font-semibold text-primary uppercase">{article.category_name}</span>}
